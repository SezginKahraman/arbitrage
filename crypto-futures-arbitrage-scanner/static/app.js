class FuturesArbitrageScanner {
    constructor() {
        ...
        this.sourcePricePrecisions = new Map();
        this.ws = null;
    ...
    formatPrice(price) {
        return PriceFormatting.formatPrice(price);
    }
    ...
    updateSourcePrice(source, price) {
        ...
        const precision = PriceFormatting.precisionForPrice(price);
        if (this.sourcePricePrecisions.get(source) !== precision) {
            const series = this.chartSeries.get(source);
            if (series) {
                series.applyOptions({
                    priceFormat: PriceFormatting.chartPriceFormat(price),
                });
            }
            this.sourcePricePrecisions.set(source, precision);
        }
    ...
    changeSymbol(newSymbol) {
        ...
        this.arbitrageOpportunities = [];
        this.sourcePricePrecisions.clear();
