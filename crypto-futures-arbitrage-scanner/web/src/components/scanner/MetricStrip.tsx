import { Activity, Radio, SlidersHorizontal, TrendingUp } from 'lucide-react';
...
export function MetricStrip({ activeOpportunities, bestSpread, freshSources, totalSources, minSpread }: MetricStripProps) {
  return (
    <section aria-label="Scanner metrics" className="flex gap-3 overflow-x-auto pb-1">
      ...
      <article className={metricClass}>
        <div className="flex items-center justify-between text-sm text-slate-400"><span>Sources online</span><Radio size={20} /></div>
        <p className="mt-2 font-data text-2xl text-signal-mint">{freshSources} / {totalSources}</p>
      </article>
      <article className={metricClass}>
