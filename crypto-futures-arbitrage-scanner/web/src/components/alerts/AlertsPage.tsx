  ...
  return (
    <section className="rounded-xl border border-terminal-line bg-terminal-panel/65 p-4">
      ...
      <form className="grid gap-3 sm:grid-cols-2" onSubmit={submit}>
        ...
        <label className="flex items-center gap-2 text-xs text-slate-700 sm:col-span-2"><input checked={draft.browserEnabled} className="accent-emerald-700" onChange={(event) => update('browserEnabled', event.target.checked)} type="checkbox" /> Browser notification when permission is granted</label>
        {error && <p className="text-xs text-red-400 sm:col-span-2" role="alert">{error}</p>}
        <button className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-signal-mint px-4 text-sm font-semibold text-white hover:brightness-110 disabled:opacity-50 sm:col-span-2" disabled={saving} type="submit"><Plus size={16} />{saving ? 'Saving…' : 'Save alert'}</button>
      </form>
    </section>
  );
}
