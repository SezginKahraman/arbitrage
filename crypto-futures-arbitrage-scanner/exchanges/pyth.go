...
func ConnectPythPrices(symbols []string, priceChan chan<- PriceData, orderbookChan chan<- OrderbookData, tradeChan chan<- TradeData, statusChan chan<- ConnectionStatus) {
	...
	if len(priceFeedIDs) == 0 {
		publishConnectionStatus(statusChan, "pyth", false, validSymbols)
	...
	for {
		...
		publishConnectionStatus(statusChan, "pyth", true, validSymbols)
